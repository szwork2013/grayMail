...
(function (jQuery, _, M139) {
        ...
        */
    {
        ...
        _initEditorHeight : function(){
        	...
        	var sendersHeight = 28;// 发件人距离父元素高度
        	if (!$D.isHide($("#moreOptions")[0])) {
        		sendersHeight = 63;
        	}
        	var bottomOperateTabHeight = 36;// 底部发送操作栏距离父元素高度
        	var divEdBody = this._getEditorBody();
        	var height = parseInt(composeIframe.style.height) - divEdBody.offset().top - sendersHeight - bottomOperateTabHeight - 36 - 8;
        ...
        _initRightContactHeight: function () {
            ...
            var groupList = divAddr.find('ul.GroupList');
            var contactListNew = divAddr.find('div.contactListNew');
            var searchEnd = divAddr.find('div.searchEnd');
            var groupListHeight = divEdBody.outerHeight(true) + divEdBody.offset().top - groupList.offset().top - contactListNew.outerHeight(true) - 1;
            groupList.height(groupListHeight);
            searchEnd.height(groupListHeight);
        },

        // 加载签名
        _loadSign : function(pageType){
        	var self = this;
		    if (pageType == "compose" || pageType == "uploadLargeAttach" || pageType == "reply" || pageType == "replyAll" || pageType == "forward" || pageType == "forwardAsAttach" || pageType == "forwardMore") {
		...
		_getVcardContent : function(userInfo,isAutoDate){
		    ...
		    if(userInfo.UserJob) contentArr.push('<tr><td width="40">职务：</td><td>'+encode(userInfo.UserJob)+'</td></tr>');
		    if(userInfo.CPName) contentArr.push('<tr><td width="40">公司：</td><td>'+encode(userInfo.CPName)+'</td></tr>');
		    if(userInfo.CPAddress) contentArr.push('<tr><td width="40">地址：</td><td>'+encode(userInfo.CPAddress)+'</td></tr>');
		    if(userInfo.FamilyEmail) contentArr.push('<tr><td width="40">邮箱：</td><td>'+encode(userInfo.FamilyEmail)+'</td></tr>');
		    if(userInfo.MobilePhone) contentArr.push('<tr><td width="40">手机：</td><td>'+encode(userInfo.MobilePhone)+'</td></tr>');
		    if(userInfo.OtherPhone) contentArr.push('<tr><td width="40">电话：</td><td>'+encode(userInfo.OtherPhone)+'</td></tr>');
		    if(userInfo.BusinessFax) contentArr.push('<tr><td width="40">传真：</td><td>'+encode(userInfo.BusinessFax)+'</td></tr>');
		    if(userInfo.CPZipCode) contentArr.push('<tr><td width="40">邮编：</td><td>'+encode(userInfo.CPZipCode)+'</td></tr>');
		    if(isAutoDate) contentArr.push('<tr><td width="40">日期：</td><td>'+$Date.format("yyyy年MM月dd日 星期w",new Date())+'</td></tr>');
		    contentArr.push('</tbody>');
		...
		_getContactImage : function(imgurl){
		    ...
		    if(imgurl && imgurl.toLowerCase() != sysImgPath[0] && imgurl.toLowerCase() != sysImgPath[1]){
		        result = top.M2012.Contacts.API.getContactsImageUrl(imgurl);
		    }else{
