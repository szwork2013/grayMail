...
function EditorBox(frameElement,plainTextElement){
    ...
    this.setLink=function(){
        if(getSelectedText(frameElement.contentWindow)==""){
            FloatingFrame.alert(FrameworkMessage.NeweditorSelError);
            return;
        }
...
function showFaceWindow(host){
    if(!theEditorBox.isHtml){
        return FloatingFrame.alert(FrameworkMessage.NeweditorTxtError);
    }
    ...
    if(window.FaceMenu){
        ...
        Utils.requestByScript("faceMenuScript",rmResourcePath+"/js/facemenu.js",function(){
