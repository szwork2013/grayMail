...
var fsConfig = {
    ...
    url: {
        rd: {
//            mailUrl     : "http://app.mail.10086rd.cn",
            loginUrl    : "http://mail.10086rd.cn/Login/login.ashx",
            //imgRndUrl   : "http://imagecode.mail.10086rd.cn/getimage?clientid=1&5377",
            autoLoginUrl: "http://mail.10086rd.cn/Login/autologin.ashx",
            ssoUrl      : "http://mail.10086rd.cn/Login/sso.aspx",
            loginHost: "http://mail.10086rd.cn/",
            cookieHost: "mail.10086rd.cn",
            serverPath: "/mw2/disk/disk?"
        },
        ts: {
//            mailUrl     : "http://app.mail.10086ts.cn",
            loginUrl: "http://mail.10086ts.cn/Login/login.ashx",
            //imgRndUrl   : "http://imagecode.mail.10086ts.cn/getimage?clientid=1&5377",
            autoLoginUrl: "http://mail.10086ts.cn/Login/autologin.ashx",
            ssoUrl      : "http://mail.10086ts.cn/Login/sso.aspx",
            loginHost   : "http://mail.10086ts.cn/",
            cookieHost: "mail.10086ts.cn",
            serverPath: "/mw2/file/disk?"
        },
        pro: {//生产线
//            mailUrl     : "http://app.mail.10086.cn",
//            mailGrayUrl : "http://app3.mail.10086.cn",
            loginUrl    : "http://mail.10086.cn/Login/login.ashx",
            //imgRndUrl   : "http://imagecode.mail.10086.cn/getimage?clientid=1&5377",
            autoLoginUrl: "http://mail.10086.cn/Login/autologin.ashx",
            ssoUrl      : "http://mail.10086.cn/Login/sso.aspx",
            loginHost   : "http://mail.10086.cn/",
            cookieHost: "mail.10086.cn",
            serverPath: "/mw2/file/disk?"
        }
    }
};
(function() {
    var partConfig = {
        "0": {
            ...
            userconfigDomain: "http://g0.mail.10086.cn",
            coremailDomain: "http://wmail-test1.mail.10086.cn",
            serverPath: "http://" + window.location.host + "/largeattachments/webservice/largservices.ashx",
            isRichmail: false
        },
        "1": {
            stylePath: "/rm",
            lighttpUrl: "http://image0.139cm.com",
            userLoginDomain: "http://mail.10086.cn",
            userconfigDomain: "http://smsrebuild0.mail.10086.cn/disk",
            coremailDomain: "http://appmail3.mail.10086.cn",
            serverPath: "http://smsrebuild0.mail.10086.cn/disk/disk",
            isRichmail: true
        },
        "11": {
            stylePath: "/m",
            ...
            userLoginDomain: "http://mail.10086.cn",
            userconfigDomain: "http://g1.mail.10086.cn",
            coremailDomain: "http://wmsvr1.mail.10086.cn",
            serverPath: "http://" + window.location.host + "/largeattachments/webservice/largservices.ashx",
            isRichmail: false
        },
        "12": {
            ...
            lighttpUrl: "http://images.139cm.com",
            userLoginDomain: "http://mail.10086.cn",
            userconfigDomain: "http://smsrebuild1.mail.10086.cn/disk",
            coremailDomain: "http://appmail.mail.10086.cn",
            serverPath: "http://smsrebuild1.mail.10086.cn/disk/disk",
            isRichmail: true
        }
    };
...
var fileSharing = {
	...
	//	return "http://appmail3.mail.10086.cn/";
    ...
    initialize: function(callback, notGlobal) {
     ...
     //   }
        fileSharing.loadScript(resourcePath + "/m2012/js/service/largeattach/jquery.js");
        fileSharing.loadScript(resourcePath + "/m2012/js/service/largeattach/utils_controls.js");
        fileSharing.loadScript("/m2012/js/service/largeattach/all.js");
        //回调函数用于加载其他脚本和样式
        if (callback) { callback(); }
    },
    //加载样式
    ...
    tool: {
        //获取当前环境是研发线，测试线或者现网，供文件提取中心使用
        getCurCondition: function(){
            var host = location.host;
            var condition = "";

            if (host.indexOf("rd") > -1) {
                condition = "rd";
				fsConfig["ajaxurl"] = '/mw2/disk/disk';
            } else if (host.indexOf("ts") > -1) {
                condition = "ts";
				fsConfig["ajaxurl"] = 'http://rm.mail.10086ts.cn/mw2/file/disk?';
            } else {
                condition = "pro";//现网
            }
			if(condition == "pro"){
				if(host.indexOf("appmail3") > -1 || location.href.indexOf("share0") > -1 || location.href.indexOf("share20") > -1){
					fsConfig["ajaxurl"] = 'http://smsrebuild0.mail.10086.cn/file/disk?';
				}else{
					fsConfig["ajaxurl"] = 'http://smsrebuild1.mail.10086.cn/file/disk?';
				}
			
			}
            return condition;
        },
        resetConfigUrl: function(){
            var condition = this.getCurCondition();
            var curUrl = fsConfig.url[condition];
        ...
        //弹出文件选择框
        showSelectBox: function(mode) {
            var url = fileSharing.tool.getRelativeUrl("selectbox.htm?sid={0}".format(fileSharing.tool.getUserInfo()));
            if (mode) {
                url += mode;
            }
            if (top._uploadWindow && !top._uploadWindow.isDisposed) {
                ...
                top._uploadWindow = fileSharing.FF.open("选择文件", url, 520, 400, true, true, true);
            }
        },
        //获取选中的文件
...
(function(){
    fileSharing.tool.resetConfigUrl();
})();
