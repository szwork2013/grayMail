...
//				             '<span>阳朔旅游</span>',
		...
		//	debugger;
			...
			if(parentDirs && parentDirs.length > 0){
				$(parentDirs).each(function(i){
					if(this.directoryName && this.directoryName.length > 5){
						this.directoryName = this.directoryName.substring(0,5);
					}
					if(curDirObj.directoryName && curDirObj.directoryName.length > 5){
						curDirObj.directoryName = curDirObj.directoryName.substring(0,5);
					}
					if(i == 0){
		...
		initNavEvents : function(){
			...
			//如果超过5项，只显示开头和结尾2项目
			var aes = $("#navContainer a");
			if(aes.length > 4){
				for(var i = 2; i < aes.length -3 + 1; i++){
					aes.eq(i).replaceWith("<span class='hi'>...</span>");
				}
			}
			var his = $("#navContainer span.hi");
			var hisnext = his.next();
			for(var k = 1; k < hisnext.length; k++){
				hisnext.eq(k).hide();
			}
...
//                            self.model.trigger('openDir', self.model.get('curDirId'));
	        ...
	        self.model.on("play", function () {// todo 播放音乐文件
	    		self.musicView.play();
	        });
	        ...
	        self.model.on("previewFile", function (fid) {// 文件预览
	        	self.fileListView.previewFile(fid);
	        });
			...
			$("#confirm").click(function(){
					self.model.setComeFrom(self.model.get('selectedFids'), self.model.viewTypes['DISK']);
					
					var filesId = self.model.get('selectedFids');
					var files = [];
					$.each(filesId, function(index,value){
						var file = self.model.getFileByIdFromAll(value);
						if(file){
							file.comeFrom = "disk";
							files.push(file);
						}
					...
					if(self && self.model){
						self.model.get('selectedFids').length = 0;
					}
			});
			$("#cancel").click(function(){
				//var selectFileDialog2 = $(top.document).find("iframe[id^='compose']")[0].contentWindow.selectFileDialog2;
				top.selectFileDialog2.close();
			});
        ...
        bindScroll :function() {
			...
			$('#fileList').bind('scroll',function(){
				console.log($(this).find('table').height());
				console.log($(this).scrollTop());
				if(($(this).find('table').height() - $(this).scrollTop()) <= 400){
					if(self.model.get('pageIndex') == self.model.getPageCount()){
						//$('#fileList').unbind('scroll');
						return;
					}
					self.model.set('pageIndex', self.model.get('pageIndex')+1);
					$('#fileList').unbind('scroll');
					setTimeout(function(){self.bindScroll()}, 1000);
				}
			})
        },
		render: function(){
            ...
            self.getIndexDisk(function(){
				...
				$(".inboxHeader.bgPadding").show();
				$("#confirmAndcancel").show();
				$(".diskTableList.onScollTable").show();
		    	self.fileListView.render();
			//	self.model.trigger('createPager');
		...
		showGuidefirstTime : function(){
			...
			var firstGuideTips = '<span class="promptOne" id="firstGuideTips"><a href="javascript:void(0);" id="firstGuideTipsClose"></a></span>';
						...
						if(!top.$App.getUserCustomInfo("sfgt")){
		...
		getIndexDisk : function(callback){
            ...
            self.model.getIndexDisk(function(result){
                ...
                if(responseData && responseData.code == 'S_OK'){
					...
					self.model.totalFileList = self.model.totalFileList.concat(self.model.diskAllInfo.fileList);
                    callback && callback();
                    ...
                    if(!self.model.get('curDirId')){
						...
						self.model.totalCurDirId.push(initializeDirid);
                    }
                }else{
		    ...
		    self.model.getDirFilesByPage(function(result){
				if(result.responseData && result.responseData.code == 'S_OK'){
					if(self.model.get("pageIndex")!=1){
						var data = result.responseData['var'].files;
						for(var i = 0; i<data.length ;i++){
							self.model.diskAllInfo.files.push(data[i])
						}
						$('#selectAll').attr('checked',false);
					}else{
						self.model.diskAllInfo = result.responseData['var'];
					...
					var curd = self.model.get('curDirId');
					var total = self.model.totalCurDirId;
					
					//if($.inArray(curd, total) == -1){
						self.model.totalFileList = self.model.totalFileList.concat(self.model.diskAllInfo.files);
						//self.model.totalCurDirId.push(curd);
					//}
                    callback && callback();
...
(function(jQuery, Backbone, _, M139) {
	...
	/**@lends M2012.MainApplication.prototype*/
	{
		...
		/**主函数入口*/
		run : function() {
			var diskModel = new M2012.Disk.Model();
			var options = {
				model : diskModel
			};
